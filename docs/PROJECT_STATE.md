...
- **Analítica (`c23d5ee`)**: sin cookies ni terceros. El juego manda `open` (carga), `play` (inicio de partida) y `end` (muerte: puntos, distancia, duración) a `/api/hit` con `sendBeacon`; el Worker añade país/ciudad/región de `request.cf` y tipo/SO/navegador del user-agent (`parseUA`). Tabla `events`. `/admin` muestra arriba: KPIs (visitas, dispositivos distintos, repetidores = dispositivos con visitas en ≥ 2 días, partidas empezadas/acabadas, hoy, medias de 30 días), gráficas SVG sin librerías (`barChart`, `hBars`; paleta dataviz modo oscuro: azul `#3987e5` visitas, naranja `#d95926` partidas, aqua `#199e70`), por día, por hora (Madrid, calculado en JS con `Intl`), países, ciudades, dispositivo, SO, navegador, idioma, referer, partidas por dispositivo. `end` se registra en cada muerte (un revive cuenta dos).
- **Modo master (`c23d5ee`)**: las teclas de prueba (1-7, Z X C V B M, N) no hacen nada hasta teclear **Q Q W W** seguidas (< 3 s, no dentro de un campo de texto); entonces sale "MASTER" y aparece el panel de teclas. Dura hasta recargar. Así nadie activa trucos por accidente en la jornada.
- **Trazabilidad de puntuaciones (24 sep, `939dbd0`…)**: cada partida guarda de dónde salieron los puntos (`src`: monedas, pacientes, salto, letras, palabras, material, mastery), el multiplicador diario (`mult`), si se jugó con las teclas de prueba (`debug`), dispositivo, monedas, pacientes y distancia. El Worker marca en `/admin` (nunca rechaza, nunca sale en el ranking público): **1** puntos/metro imposibles (> 400; medido con un bot dentro del juego: partida normal ~1,5 p/m, con el mejor power-up permanente ~7, abusando de las teclas de prueba ~240), **2** teclas de prueba, **3** el desglose no suma el total (el número no lo produjo el juego), **4** sin desglose. Límite: el desglose lo calcula el mismo cliente, así que detecta fallos del juego y manipulaciones caseras, no a alguien decidido a falsificar la petición entera.
- **Caso resuelto (25 sep, `9b7d84b`, `4f2ac61`)**: las puntuaciones "imposibles" **no eran trampas**, era un fallo del multiplicador diario. Al cerrar una partida se añadían tantas misiones nuevas como completadas sin quitar las sobrantes → la lista de 3 activas creció hasta 19 → 14 misiones completadas por partida → el multiplicador (1 + misiones/3, sin tope) llegaba a ×169 tras 40 partidas en un día, y una partida normal de 2,3 km daba 1.389.660 puntos. Reproducido con un bot que juega solo dentro del juego. Arreglo: siempre 3 misiones activas, curva que se frena sola `1 + 1,2·√misiones` (3 misiones ×3, 9 ×5, 30 ×8, 100 ×13, freno duro ×20) y saneado del `localStorage` al cargar. Validado con 60 partidas seguidas: máximo ×8, 86.400 puntos, 32 p/m y desglose correcto en las 60.
- **Puntuaciones de la beta convertidas a equivalente (25 sep)**: en vez de borrarlas (el usuario no quería perder nada), las 162 partidas anteriores al arreglo se recalcularon a `(monedas×10 + pacientes×100 + metros/2) × 3`, guardando la original en `scores.score_raw` (visible en /admin). Así el ranking vuelve a ser comparable y alcanzable: manda Isaac (60.483 equivalentes, 9.623 m), antes sepultado en el puesto 20.
- **Caso anterior (beta del 24 sep)**: 20 de 162 partidas superan los 400 p/m (hasta 2.166), repartidas en **7 dispositivos distintos**, y el mismo jugador alterna partidas normales (10-20 p/m) con imposibles minutos después. Con el código actual no se reproduce: un bot invencible recogiéndolo todo durante 11 km no pasa de 9 p/m. Queda pendiente ver el desglose de las próximas partidas marcadas para decidir si es un fallo en dispositivos reales o envíos manipulados.
