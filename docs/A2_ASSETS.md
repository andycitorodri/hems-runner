...
| 10b | Patinete (obstáculo `scooter`) | GENÉRICO_JUEGO | **Poly Pizza — jeremy** | CC-BY 3.0 | **INTEGRADO** | `assets/models/polypizza/scooter.glb` (28 KB, 728 tris, escala 0.083, `ground`). `ScooterInstancer` (aura + 3 mallas). Atribución en créditos. https://poly.pizza/m/aQ_Us04oiz1 |
...
| 16 | Tres Xemeneies (Sant Adrià) | Landmark (carrusel, 5.º) | Sketchfab — novusod | CC-BY 4.0 | **INTEGRADO** | `assets/models/sketchfab/tres-xemeneies.glb` (244 KB, 7k tris desde 30k; 7 texturas quitadas → 3 colores planos con `flatten-glb.mjs BUCKETS`, `weld --tolerance 0.003` + `simplify --ratio 0.3 --error 0.03` + `optimize-glb.sh`). |
